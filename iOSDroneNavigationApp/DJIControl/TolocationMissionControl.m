...
#import "TolocationMissionControl.h"
#import <DJISDK/DJISDK.h>
#import "DemoUtility.h"

#define ONE_METER_OFFSET (0.00000901315)

@interface TolocationMissionControl()
...
@implementation TolocationMissionControl
...
-(void)createWayPointMissionByWaypointMission:(DJIWaypointMission *)mission {
    NSError *error = [self.wpOperator loadMission:mission];
    if (error) {
        ShowResult(@"Prepare Mission Failed:%@", error);
        return;
    }
...
//    WeakRef(target);
    [self.wpOperator addListenerToExecutionEvent:self withQueue:nil andBlock:^(DJIWaypointMissionExecutionEvent * _Nonnull event) {
        [weakSelf showWaypointMissionProgress:event];
    }];
